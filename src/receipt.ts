...
function codeBlock(value: string | undefined, empty: string, label: string): string {
  return value ? `<pre tabindex="0" aria-label="${escapeHtml(label)}. Scroll to inspect all content."><code>${escapeHtml(value)}</code></pre>` : `<p class="empty">${escapeHtml(empty)}</p>`;
}
...
<header><span class="stamp">Assertion failed · receipt saved</span><h1>${escapeHtml(evidence.label)}</h1><p class="error">${escapeHtml(evidence.error)}</p><dl>${rows([['Test', evidence.testTitle], ['Project', evidence.projectName || 'default'], ['Captured', evidence.capturedAt], ['Page', evidence.pageUrl], ['Receipt', evidence.id]])}</dl></header>
...
<section class="sheet" aria-labelledby="structure-title"><h2 id="structure-title">02 · Structure</h2><details open><summary>Scrubbed DOM</summary>${codeBlock(evidence.dom, 'No DOM snapshot was available.', 'Scrubbed DOM capture')}</details><details><summary>ARIA snapshot</summary>${codeBlock(evidence.aria, 'No ARIA snapshot was available.', 'ARIA snapshot')}</details></section>
...
<footer><p class="privacy">Local receipt · request and response bodies excluded · URL paths and query strings discarded</p><p>Generated by Journey Failure Receipts 0.1.4.</p></footer>
</main></body></html>`;
}
